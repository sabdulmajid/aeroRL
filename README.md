...
## Model-generated benchmark (real images + real VLM output)

We also ran a benchmark where responses are generated by a cached VLM on GPU (not deterministic corruption patterns):
- model: `HuggingFaceTB/SmolVLM-256M-Instruct`
- device: `cuda:0`
- datasets: cached DocVQA + ChartQA image/question rows

Run command:

```bash
HF_HOME=/pub7/neel2/.cache_hf HUGGINGFACE_HUB_CACHE=/pub7/neel2/.cache_hf/hub TRANSFORMERS_CACHE=/pub7/neel2/.cache_hf/hub \
python benchmarks/reward_model_generated_benchmark.py \
	--device cuda:0 \
	--cache-dir /pub7/neel2/.cache_hf \
	--limit-docvqa 300 \
	--limit-chartqa 500 \
	--max-new-tokens 16 \
	--output reports/reward-model-generated-benchmark-2026-03-24-large.json \
	--replay-output reports/reward-model-generated-replay-2026-03-24-large.jsonl
```

Measured result (`reports/reward-model-generated-benchmark-2026-03-24-large.json`):

| Metric | Manual baseline | AeroRL stack | Improvement shown |
|---|---:|---:|---|
| Total real model-generated samples | 800 | 800 | Same run |
| Pass rate | 0.47125 | 0.2875 | AeroRL is stricter on quality |
| Hidden false passes caught | 0 | 147 | Manual misses grounded failures |
| Hidden false-pass fraction (manual passes) | N/A | 0.38992 | ~38.99% manual passes were low-quality |
| Generation throughput | N/A | 1.462 samples/s (GPU) | Practical for larger replay-scale scoring |

GPU safety/usage from the same report:
- before run: GPU0 `67 MiB`, `0%` util
- after run: GPU0 `96328 MiB`, `96%` util; GPU1 `96503 MiB`, `99%` util

Why this benchmark matters:
- It uses real image-conditioned model outputs.
- It keeps the same manual-vs-AeroRL comparison contract.
- It shows AeroRL still catches hidden failures when generation is truly model-produced.
